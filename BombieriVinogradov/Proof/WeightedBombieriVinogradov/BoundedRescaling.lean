...
/-- Rescale one finite endpoint bound uniformly over a positive compact range. -/
theorem bounded_weighted_rescaling {X T A K : Real}
    (hX : 3 <= X) (hXT : X <= T) (hA : 1 <= A) (hK : 0 <= K) :
    K <= ((K + 1) * (Real.log T) ^ A) * (X / (Real.log X) ^ A) := by
  ...
  have hLogOrder : Real.log X <= Real.log T := Real.log_le_log hXPos hXT
  ...
  have hProduct : (Real.log X) ^ A <= (Real.log T) ^ A * X := by
    calc
      (Real.log X) ^ A = (Real.log X) ^ A * 1 := by ring
        _ <= (Real.log T) ^ A * X :=
        mul_le_mul hPower hXOne (by linarith) (Real.rpow_nonneg (by linarith) A)
  have hScale : 1 <= (Real.log T) ^ A * (X / (Real.log X) ^ A) := by
  ...
  calc
    K <= K + 1 := by linarith
    _ = (K + 1) * 1 := by ring
    _ <= (K + 1) * ((Real.log T) ^ A * (X / (Real.log X) ^ A)) :=
      mul_le_mul_of_nonneg_left hScale (by linarith)
    _ = ((K + 1) * (Real.log T) ^ A) * (X / (Real.log X) ^ A) := by ring
