...
theorem meromorphicOrderAt_nonneg_of_isBigO_one
    ...
    (hO : f =O[nhdsWithin p (Set.compl ({p} : Set Complex))]
      (1 : Complex -> Complex)) :
    0 <= meromorphicOrderAt f p := by
  let _ : Filter.NeBot (nhdsWithin p (Set.compl ({p} : Set Complex))) :=
    NormedField.nhdsNE_neBot p
  by_contra hnonneg
  have hneg : meromorphicOrderAt f p < 0 := lt_of_not_ge hnonneg
  have hnorm :
      Filter.Tendsto (fun z : Complex => norm (f z))
        (nhdsWithin p (Set.compl ({p} : Set Complex))) Filter.atTop := by
